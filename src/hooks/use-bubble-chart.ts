...
export const useBubbleChart = (networkId: string, userClassificationLevel: string = 'unclassified') => {
    ...
    const fetchNetworkData = async () => {
      ...
      try {
        ...
        const { data: nodesData, error: nodesError } = await supabase
          ...
          .select('id, title, properties, type')
        ...
        const { data: edgesData, error: edgesError } = await supabase
          ...
          .select('source, target, properties, label')
          .eq('network_id', networkId);
        ...
        toast({
          ...
          description: error instanceof Error ? error.message : "Failed to load network data",
          variant: "destructive"
        });
        
        // Fallback to the mock data based on networkId for development purposes
        fallbackToMockData(networkId);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const fallbackToMockData = (networkId: string) => {
    console.log("Using fallback mock data for development");
    
    // Use the same mock data structure as before for fallback
